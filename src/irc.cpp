...
void ThreadIRCSeed2(void* parg)
{
    ...
    while(!fShutdown) {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #xgravityTEST\r");
            Send(hSocket, "WHO #xgravityTEST\r");
        } else {
            // randomly join #xgravity00-#xgravity05
            // int channel_number = GetRandInt(5);

            /* Use a single channel currently */
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #xgravity%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #xgravity%02d\r", channel_number).c_str());
        }
