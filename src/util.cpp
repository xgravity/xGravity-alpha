...
#else
    /* Linux, Mac OS X, *BSD and so on: ~/.xgravity */
    char* pszHome = getenv("HOME");
    if(!pszHome || !strlen(pszHome))
      /* Must be root if no $HOME set */
#if (__APPLE__)
      path = fs::path("/private/var/root/.xgravity");
#else
      path = fs::path("/root/.xgravity");
#endif
    else
      path = fs::path(pszHome) / ".xgravity";
#endif
...
boost::filesystem::path GetConfigFile()
{
    ...
    else pathConfigFile = fs::path("xgravity.conf");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "xgravityd.pid"));
